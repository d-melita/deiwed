<template>
    <v-card :flat="$store.getters.isMobile" class="table pb-4">
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="MakeOrder()">Fazer encomenda</v-btn>
        </v-card-actions>
    </v-card>
</template>
...
export default class SessionMakeOrderView extends Vue {
    ...
    MakeOrder() {
        RemoteServices.makeOrder(this.normalOptionDishId, this.vegetarianOptionDishId, this.session.date);
